...
async function update(x) {
	...
	apiCall("POST", { "username": username, "action": 1, "time": currentTime, "ep": data_main.episode, "name": data_main.nameWSeason}, (x)=> {}, [], true).then(function (x) {
		try{
...
async function get_ep(x = 0) {
	...
	try {
		...
		let response = await apiCall("POST", { "username": username, "action": 2, "name": data_main.nameWSeason, "nameUm": data_main.name, "ep": data_main.episode, "cur": cur_link }, (x) => {});
